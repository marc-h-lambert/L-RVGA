...
def XP_HighDim_LinReg_Lattent(axs,sigma0,mu0,sigmaNoise,N,d,list_p,c,ppca,svd,nbInnerLoop,seed,label=True):
    ...
    eps=1e-8
    psi0=np.ones([d,1])*(1-eps)*1/(sigma0**2) 
    list_lskf=[]
    for p in list_p:
        np.random.seed(seed)
        if p==0 or svd:
            ...
            for i in range(0,p):
                w0=math.sqrt(eps*d/p)/sigma0
                W0[:,i]=W0[:,i]*w0/LA.norm(W0[:,i])

        tic=time.perf_counter()
        lskf = LargeScaleLKFLinReg(theta0, psi0, W0, passNumber=1, sigma=1, ppca=ppca, svd=False, \
...
if __name__=="__main__":
    Test=["LinHD"] # change the label to change the Section tested
    num=1    
